...
parser.add_argument('--learning_rate_steps', type=list, default=[50], help='learning_rate_steps')
...
def Bi_Loss(bi_pred, bi_label):
    ...
    ratio_0 = ((sum - label_0.shape[0]) / sum)*5.5
...
def Multi_Loss(multi_pred, multi_label):
    ...
    label_6 = torch.nonzero(multi_label == 6).cuda()
    label_7 = torch.nonzero(multi_label == 7).cuda()
    label_8 = torch.nonzero(multi_label == 8).cuda()
    ...
    ratio_6 = ((sum - label_6.shape[0]) / sum)
    ratio_7 = ((sum - label_7.shape[0]) / sum)
    ratio_8 = ((sum - label_8.shape[0]) / sum)
    ...
    ratio_6_f = torch.ones([sum, 1]) * ratio_6
    ratio_7_f = torch.ones([sum, 1]) * ratio_7
    ratio_8_f = torch.ones([sum, 1]) * ratio_8

    ratio = torch.cat((ratio_0_f, ratio_1_f, ratio_2_f, ratio_3_f, ratio_4_f, ratio_5_f, ratio_6_f, ratio_7_f, ratio_8_f), dim=1).cuda()
    multi_target = torch.eye(9)[multi_label, :].cuda()
...
class Trainer(object):
    ...
    def train(self):
        ...
        train_dataset = SpaceNet7_custom.MyDataset(csv_train, Ftrain, args.patch_size)
        train_sample = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=False, drop_last=True)
        val_dataste = SpaceNet7_custom.MyDataset(csv_val, Fval, args.patch_size)
        ...
        for epoch in tqdm(range(0, args.max_epoch)):
            ...
            Train_Bi_Matrxi = get_f1.bi_matrix(train_bi_pred_conf, train_bi_target_conf)
            Train_Multi_Matrix = get_f1.multi_matrix(train_multi_pred_conf, train_multi_target_conf)
            Val_Bi_Matrix = get_f1.bi_matrix(val_bi_pred_conf, val_bi_target_conf)
            Val_Multi_Matrix = get_f1.multi_matrix(val_multi_pred_conf, val_multi_target_conf)
            ...
            best_model1 = args.save_path + '/model_' + str(epoch) + '.pth'
