...
class MyDataset(Dataset):
    def __init__(self, csv_path, image_ids, patch_size):
        ...
        for fold in image_ids:
            all_tifs = glob.glob( fold[4:] + '/train_images/20*.tif*')
            years = []
            for j in range(len(all_tifs)):
                ff = all_tifs[j].find('train_images/20')
                years.append(all_tifs[j][ff+13:ff+17]+all_tifs[j][ff+18:ff+20])
        ...
        self.bi_labels = []
        for fold in image_ids:
            bi_label = io.imread(fold[4:] + '/change/bi_change.tif')
            self.bi_labels.append(bi_label)

        self.multi_labels = []
        for fold in image_ids:
            multi_label = io.imread(fold[4:] + '/change/multi_change.tif')
